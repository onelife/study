...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.length == 0) means.map(p => p -> points).toMap
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall(x => math.sqrt(
        math.pow(x._1.x - x._2.x, 2) + 
        math.pow(x._1.y - x._2.y, 2) + 
        math.pow(x._1.z - x._2.z, 2)) < eta)
  }
