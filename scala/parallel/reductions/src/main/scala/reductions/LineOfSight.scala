...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while(i < input.length) {
      output(i) = max(output(i - 1), input(i) / i.toFloat)
      i += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while(i < until) {
      maxAngle = max(maxAngle, input(i) / i.toFloat)
      i += 1
    }
    maxAngle
  }
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    threshold: Int): Unit = {
    output(0) = 0
    //println(upsweep(input, 1, input.length, threshold))
