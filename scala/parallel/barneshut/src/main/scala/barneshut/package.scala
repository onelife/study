...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val s = size / 4
        var fork = new Fork(
            new Empty(centerX - s, centerY - s, s * 2), 
            new Empty(centerX + s, centerY - s, s * 2),  
            new Empty(centerX - s, centerY + s, s * 2),  
            new Empty(centerX + s, centerY + s, s * 2)
            )
        for (x <- bodies) fork = fork.insert(x)
        fork.insert(b)
      } else 
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        case Empty(_, _, _) =>
          // no force
        case Leaf(_, _, _, bodies) => for (b <- bodies) {
        ...
        case fork @ Fork(nw, ne, sw, se) => {
          ...
          val dist = distance(x, y, fork.centerX, fork.centerY)
          if (fork.size / dist < theta)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x0 = (b.x - boundaries.minX) / (boundaries.width / sectorPrecision)
      val x = if (x0 < 0) 0 
              else if (x0 > sectorPrecision - 1) sectorPrecision - 1
              else x0.toInt
      val y0 = (b.y - boundaries.minY) / (boundaries.height / sectorPrecision)
      val y = if (y0 < 0) 0 
              else if (y0 > sectorPrecision - 1) sectorPrecision - 1
              else y0.toInt
