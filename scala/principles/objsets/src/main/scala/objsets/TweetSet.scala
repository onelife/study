...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = 
    (if (p(elem)) acc incl elem else acc) union left.filterAcc(p, acc) union right.filterAcc(p, acc)
  ...
  def mostRetweeted: Tweet = {
    ...
    val m = if (l == null) r else if (r == null || l.retweets > r.retweets) l else r
    if (m == null || elem.retweets > m.retweets) elem else m
  }
  
  def descendingByRetweet: TweetList = {
    var tw = mostRetweeted
    if (tw == null) Nil
    else new Cons(tw, this.remove(tw).descendingByRetweet)
  }
