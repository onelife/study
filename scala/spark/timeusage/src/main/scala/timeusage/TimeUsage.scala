...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row.fromSeq(line.head :: line.tail.map(data => data.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    for (name <- columnNames) {
      if (primaryPrefix.exists(name.startsWith))
        primary = primary :+ col(name)
      else if (workPrefix.exists(name.startsWith))
        work = work :+ col(name)
      else if (otherPrefix.exists(name.startsWith))
        other = other :+ col(name)
    }
    (primary, work, other)
  }
...
//      expr("case when 1 <= telfs and telfs < 3 then \"working\" else \"not working\" end").as("working")
    val sexProjection: Column = when('tesex === 1, "male").otherwise("female").as("sex")
//      expr("case when tesex = 1 then \"male\" else \"female\" end").as("sex")
    val ageProjection: Column = when('teage.between(15.0, 22.0), "young").when('teage.between(23.0, 55.0), "active").otherwise("elder").as("age")
//      expr("case when 15 <= teage and teage <= 22 then \"young\" when 23 <= teage and teage <= 55 then \"active\" else \"elder\" end").as("age")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]) = {
    ...
    def round_1(num: Double): Double = (num * 10).round.toDouble / 10
    ...
    summed
      ...
      .map({
        case ((working, sex, age), primaryNeeds, work, other) => 
          TimeUsageRow(working, sex, age, round_1(primaryNeeds), round_1(work), round_1(other))
      })
